...
// - value(슬러그, null=기본값 따라감) / onChange(슬러그) 로 부모가 제어하는 controlled 컴포넌트.
// - models가 null이면(로딩 중·조회 실패) 비활성화하고 현재 값만 보여준다 —
//   프론트에 폴백 카탈로그를 두지 않는다(백엔드와 어긋난 복사본 방지).
export default function ModelPicker({ models, defaultModel, value, onChange, disabled = false, id }) {
  // value가 null이면(방에 저장된 모델 없음) 백엔드가 알려준 기본 모델을 보여준다.
  ...
  return (
    <select
      ...
      aria-label="대화 모델"
    >
      {options.length === 0 && <option value="">기본 모델</option>}
      {options.map((m) => (
